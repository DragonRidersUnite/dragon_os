class FallingCircle
  ...
  def defaults
    if state.tick_count == 0
      outputs.sounds << "app/the_little_probe/sounds/bg.ogg"
    end
  ...
  def render_game
    ...
    outputs.sprites << [-circle.x + 1100,
                        ...
                        2416 * 4,
                        3574 * 4,
                        'app/the_little_probe/sprites/jupiter.png']
    outputs.sprites << [-circle.x,
                        -circle.y,
                        2416 * 4,
                        3574 * 4,
                        'app/the_little_probe/sprites/level.png']
  ...
  def render_aiming_retical
    outputs.sprites << [state.camera.x + circle.x + circle.angle.vector_x(circle.potential_lift * 10) - 5,
                        ...
                        10, 10, 'app/the_little_probe/sprites/circle-orange.png']
    outputs.sprites << [state.camera.x + circle.x + circle.angle.vector_x(circle.radius * 3) - 5,
                        ...
                        10, 10, 'app/the_little_probe/sprites/circle-orange.png', 0, 128]
    if rand > 0.9
      outputs.sprites << [state.camera.x + circle.x + circle.angle.vector_x(circle.radius * 3) - 5,
                          state.camera.y + circle.y + circle.angle.vector_y(circle.radius * 3) - 5,
                          10, 10, 'app/the_little_probe/sprites/circle-white.png', 0, 128]
    end
  end
  ...
  def render_circle
    outputs.sprites << circle.after_images.map do |ai|
      ai.merge(x: ai.x + state.camera.x - circle.radius,
               ...
               w: circle.radius * 2,
               h: circle.radius * 2,
               path: 'app/the_little_probe/sprites/circle-white.png')
    end
    ...
    outputs.sprites << [(circle.x - circle.radius) + state.camera.x,
                        ...
                        circle.radius * 2,
                        circle.radius * 2,
                        'app/the_little_probe/sprites/probe.png']
  end
  ...
  def load_terrain
    load_lines 'app/the_little_probe/data/level.txt'
  end

  def load_lava
    load_lines 'app/the_little_probe/data/level_lava.txt'
  end
  ...
  def play_sound
    ...
    outputs.sounds << "app/the_little_probe/sounds/03#{"%02d" % state.sound_index}.wav"
    state.sound_index += 1
  ...
  def calc
    ...
    if state.tick_count.mod_zero?(4)
      state.whisp_queue << {
        ...
        w: 20,
        h: 20, path: 'app/the_little_probe/sprites/whisp.png',
        a: 0,
...
class TheLittleProbe < Game
  NAME = "The Little Probe"
