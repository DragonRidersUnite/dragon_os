class FlappyDragon < Game
  ...
  def render
    outputs.sounds << "app/flappy_dragon/sounds/flappy-song.ogg" if state.tick_count == 1
    render_score
    render_menu
    render_game
  end
  ...
  def render_background
    outputs.sprites << { x: 0, y: 0, w: 1280, h: 720, path: 'app/flappy_dragon/sprites/background.png' }
    ...
    outputs.sprites << scrolling_background(scroll_point_at, 'app/flappy_dragon/sprites/parallax_back.png',   0.25)
    outputs.sprites << scrolling_background(scroll_point_at, 'app/flappy_dragon/sprites/parallax_middle.png', 0.50)
    outputs.sprites << scrolling_background(scroll_point_at, 'app/flappy_dragon/sprites/parallax_front.png',  1.00, -80)
  end
  ...
  def render_walls
    state.walls.each do |w|
      w.sprites = [
        { x: w.x, y: w.bottom_height - 720, w: 100, h: 720, path: 'app/flappy_dragon/sprites/wall.png',       angle: 180 },
        { x: w.x, y: w.top_y,               w: 100, h: 720, path: 'app/flappy_dragon/sprites/wallbottom.png', angle: 0 }
      ]
    end
  ...
  def render_dragon
    ...
    if state.show_death == false || !state.death_at
      ...
      sprite_name = "app/flappy_dragon/sprites/dragon_fly#{animation_index.or(0) + 1}.png"
      state.dragon_sprite = { x: state.x, y: state.y, w: 100, h: 80, path: sprite_name, angle: state.dy * 1.2 }
    else
      sprite_name = "app/flappy_dragon/sprites/dragon_die.png"
      state.dragon_sprite = { x: state.x, y: state.y, w: 100, h: 80, path: sprite_name, angle: state.dy * 1.2 }
  ...
  def calc_game_over
    ...
    outputs.sounds << "app/flappy_dragon/sounds/hit-sound.wav"
    begin_countdown
  end
  ...
  def process_inputs_game
    ...
    if clicked_menu || inputs.keyboard.key_down.escape || inputs.keyboard.key_down.enter || inputs.controller_one.key_down.start
      ...
      outputs.sounds << "app/flappy_dragon/sounds/fly-sound.wav"
    end
  end
